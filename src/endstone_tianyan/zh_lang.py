...
default_lang = {
    ...
    "实验性功能 还原玩家直接方块放置破坏行为和实体爆炸破坏 --格式 /tyback 坐标 时间(单位:小时) 半径 实施行为的玩家名或爆炸的实体ID(可选)(仅管理员可用)": "实验性功能 还原玩家直接方块放置破坏行为和实体爆炸破坏 --格式 /tyback 坐标 时间(单位:小时) 半径 实施行为的玩家名或爆炸的实体ID(可选)(仅管理员可用)",
    ...
    "实验性功能 使用/tyback 命令还原玩家直接方块放置破坏行为和实体爆炸破坏 格式 /tyback 坐标 时间(单位:小时) 半径 实施行为的玩家名或爆炸的实体ID(可选)(仅管理员可用)": "实验性功能 使用/tyback 命令还原玩家直接方块放置破坏行为和实体爆炸破坏 格式 /tyback 坐标 时间(单位:小时) 半径 实施行为的玩家名或爆炸的实体ID(可选)(仅管理员可用)",
    ...
    "\n\n天眼插件更新检测:\n更新检测超时\n": "\n\n天眼插件更新检测:\n更新检测超时\n",
    "\n\n天眼插件更新检测:\n检测更新时发生未知错误": "\n\n天眼插件更新检测:\n检测更新时发生未知错误",
    "检测实体密度最高的区域 --格式 /tydensity 区域大小": "检测实体密度最高的区域 --格式 /tydensity 区域大小",
    "最高密度区域在维度": "最高密度区域在维度",
    "中点坐标": "中点坐标",
    "实体数量": "实体数量",
    "数量最多的实体": "数量最多的实体",
    ...
    "传送到该区域": "传送到该区域",
    "打印信息到聊天栏": "打印信息到聊天栏",
    "未提供参数,使用默认参数20格区域计算": "未提供参数,使用默认参数20格区域计算",
    "使用 /tydensity 命令检测实体密度最高的区域 --格式 /tydensity 区域大小 (仅管理员可用)": "使用 /tydensity 命令检测实体密度最高的区域 --格式 /tydensity 区域大小 (仅管理员可用)",
    "使用 /tyclean命令清理数据库中超过一定时间的数据 --格式 /tyclean 时间 (仅管理员可用)": "使用 /tyclean命令清理数据库中超过一定时间的数据 --格式 /tyclean 时间 (仅管理员可用)",
    "爆炸": "爆炸"
}